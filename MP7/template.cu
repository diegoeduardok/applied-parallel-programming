...
__global__ void spmvJDSKernel(float *out, int *matColStart, int *matCols,
    ...
    if (row < dim) {
        float result = 0.;
        unsigned int iter = matRows[row];
        for (int i = 0; i < iter; i++) {
            result += matData[matColStart[i] + row] * vec[matCols[matColStart[i] + row]];
        }
...
static void spmvJDS(float *out, int *matColStart, int *matCols,
    ...
    dim3 GridDim( ceil((float) dim / BLOCK_SIZE));
    dim3 BlockDim(BLOCK_SIZE);
    spmvJDSKernel<<<GridDim, BlockDim>>>(out, matColStart, matCols,
